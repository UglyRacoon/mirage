...
- На **любом** свежем Linux: one-shot `sudo ./deploy.sh` (Node 22 + chromium + Xvfb + nginx, юнит,
  vhost, `/etc/hosts`; идемпотентен). После деплоя **смените PIN** и, при удалённом доступе,
  закройте vhost за TLS + авторизацией.
